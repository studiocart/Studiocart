 ...
 * @since 1.0.0
 */
class SC_Bump_Image extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Bump Image', 'elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_image',
			[
				'label' => esc_html__( 'Bump Image', 'elementor' ),
			]
		);
		...
		$this->add_control(
			'image',
			[
				'label' => esc_html__( 'Choose a Preview Image', 'elementor' ),
	...
	protected function render() {
        ...
        if ( empty( $settings['image']['url'] ) && !isset($sc_bump['image'])) {
			return;
		}
        ...
        if(isset($sc_bump['image'])) {
            $settings['image']['id'] = attachment_url_to_postid( $sc_bump['image']);
            $settings['image']['url'] = $sc_bump['image'];
        }
...
\Elementor\Plugin::instance()->widgets_manager->register( new SC_Bump_Image() );