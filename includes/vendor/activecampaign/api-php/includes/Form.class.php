...
class AC_Form extends ActiveCampaign
{
    ...
    function process($params)
    {
        ...
        if (isset($_POST["fullname"])) {
            ...
            $firstname = \trim($_POST["firstname"]);
            $lastname = \trim($_POST["lastname"]);
            if ($firstname == "" && isset($_POST["first_name"])) {
                $firstname = \trim($_POST["first_name"]);
            }
            if ($lastname == "" && isset($_POST["last_name"])) {
                $lastname = \trim($_POST["last_name"]);
            }
        }
        $fields = isset($_POST["field"]) ? $_POST["field"] : array();
        $contact = array("form" => $formid, "email" => $email, "first_name" => $firstname, "last_name" => $lastname, "phone" => $phone);
        ...
        if (!$sync) {
            ...
            if (!isset($contact_exists->id)) {
                ...
                if ((int) $contact_request->success) {
                    ...
                    $r = array("success" => 1, "message" => $contact_request->result_message, "contact_id" => $contact_id);
                } else {
                    // request failed
                    $r = array("success" => 0, "message" => $contact_request->error);
                }
            } else {
        ...
        if ((int) $contact_request->success) {
            ...
            $r = array("success" => 1, "message" => $contact_request->result_message);
        } else {
            // request failed
            $r = array("success" => 0, "message" => $contact_request->error);
        }
...
\class_alias('NCS_Cart\\AC_Form', 'AC_Form', \false);
